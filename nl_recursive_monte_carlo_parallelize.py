...
def run_trial(trial):
    ...
    cov_hist_trial = np.zeros_like(cov_hist)  # Local copy for this trial
    fim_trial = np.zeros((N*state_dim,N*state_dim))  # Local copy for this trial
    ...
    for i in range(N):
        ...
        for sat in sats_copy:
            ...
            y_m = y_m.at[i, 0:bearing_dim, sat.id].set(
                sat.measure_z_landmark(tuple(landmark_objects)))
            y_m = y_m.at[i, bearing_dim:meas_dim, sat.id].set(
                sat.measure_z_range(i, sats))
            ...
            sat.cov_m = (np.eye(state_dim) - K @ H) @ sat.cov_p @ (
                (np.eye(state_dim) - K @ H).T) + K @ R @ K.T

            cov_hist_trial[i, sat.id, :, :] += sat.cov_m

        if i > 0:
            start_i = i * state_dim
            A = state_transition(sats_copy[0].x_m)
            f_prior = A @ f_post @ A.T + np.linalg.inv(Q)
            J[0:bearing_dim, 0:3] = sats_copy[0].H_landmark(
                sats_copy[0].x_m[0:3])
            for j in range(3, meas_dim):
                J[j, 0:3] = sats_copy[0].H_inter_range(
                    i, j, sats_copy[0].x_m[0:3])
            f_post = f_prior + J.T @ R_inv @ J
            fim_trial[start_i:start_i + state_dim,
                      start_i:start_i + state_dim] = f_post

    # Return results from this trial
    print("Done trial ",trial)
    return cov_hist_trial, fim_trial
...
if parallelize:
    ...
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(run_trial, range(num_trials)))

    # Aggregate results
    for cov_hist_trial, fim_trial in results:
        cov_hist += cov_hist_trial
        fim += fim_trial

    print("Averaging FIM")
    # Divide FIM by number of trials
    for i in range(N):
        start_i = i * state_dim
        fim[start_i:start_i+state_dim,start_i:start_i+state_dim] = fim[start_i:start_i+state_dim,start_i:start_i+state_dim]/(num_trials)
    ...
    # Get the average FIM over all trials
...
# Plot the covariance matrix entries and the FIM. 
